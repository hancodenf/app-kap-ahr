...
interface AuditIndexPageProps extends PageProps {
    clients: {
        data: Client[];
        current_page: number;
        last_page: number;
...
export default function Index({ clients, filters }: AuditIndexPageProps) {
    const { flash } = usePage().props as any;
    const [search, setSearch] = useState(filters.search || '');

    const handleSearch = () => {
        router.get(route('admin.audit.index'), { search }, {
            preserveState: true,
            replace: true,
        });
    };
                        ...
                        <div className="p-6">
                            <div className="mb-6">
                                <p className="text-gray-600 mb-4">
                                    Pilih klien untuk melihat dan mengelola data audit mereka.
                                </p>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {clients.data.map((client) => (
                                    <div key={client.id} className="bg-gray-50 rounded-lg p-6 border border-gray-200 hover:shadow-md transition-shadow">
                                        ...
                                        <Link
                                            href={route('admin.audit.show', client.id)}
