...
Route::get('/', function () {
    ...
    // Fetch latest 3 published news for the welcome page
    $latestNews = \App\Models\News::orderBy('published_at', 'desc')
        ->take(3)
        ->get()
        ->map(function ($news) {
            return [
...
// Admin Routes
Route::middleware(['auth', 'verified', 'role:admin', 'no.cache'])
    ->prefix('admin')
    ->name('admin.')
    ->group(function () {
        ...
        Route::prefix('login-security')
            ->name('login-security.')
            ->middleware('security.unlock')
            ->group(function () {
...
// Company Routes
Route::middleware(['auth', 'verified', 'role:company', 'no.cache'])
    ->prefix('company')
    ->name('company.')
    ->group(function () {
...
// Partner Routes
Route::middleware(['auth', 'verified', 'role:partner', 'no.cache'])
    ->prefix('partner')
    ->name('partner.')
    ->group(function () {
...
// Staff Routes
Route::middleware(['auth', 'verified', 'role:staff', 'no.cache'])
    ->prefix('staff')
    ->name('staff.')
    ->group(function () {
...
// Klien Routes
Route::middleware(['auth', 'verified', 'role:klien', 'no.cache'])
    ->prefix('klien')
    ->name('klien.')
    ->group(function () {
...
Route::get('/berita', [\App\Http\Controllers\Admin\NewsController::class, 'publicIndex'])->name('public.news.index');
Route::get('/berita/{news:slug}', [\App\Http\Controllers\Admin\NewsController::class, 'publicShow'])->name('public.news.show');

// Authenticated News Routes (accessible by all authenticated users)
Route::middleware(['auth', 'no.cache'])->group(function () {
